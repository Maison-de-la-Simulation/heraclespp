...
enum class access_mode : unsigned char { read_only, read_write, discard_write, invalid };

enum class access_target : unsigned char { device, host, invalid };
...
inline constexpr AccessTag<access_target::device, access_mode::read_only> read_only_device;
inline constexpr AccessTag<access_target::device, access_mode::read_write> read_write_device;
inline constexpr AccessTag<access_target::device, access_mode::discard_write> discard_write_device;
inline constexpr AccessTag<access_target::host, access_mode::read_only> read_only_host;
inline constexpr AccessTag<access_target::host, access_mode::read_write> read_write_host;
inline constexpr AccessTag<access_target::host, access_mode::discard_write> discard_write_host;

template <access_mode AM>
constexpr auto need_read_access(AccessModeTag<AM> const mode) noexcept -> bool
{
    return mode.value == access_mode::read_only || mode.value == access_mode::read_write;
}

template <access_mode AM>
constexpr auto need_write_access(AccessModeTag<AM> const mode) noexcept -> bool
{
...
template <class DataType, class... DataProperties>
class DualView : private Kokkos::ViewTraits<DataType, DataProperties...>
{
    template <class DT, class... DP>
    friend class DualView;

    using traits = Kokkos::ViewTraits<DataType, DataProperties...>;
    ...
    using modified_flag_type = Kokkos::View<unsigned int, Kokkos::LayoutLeft, Kokkos::HostSpace>;
...
public:
    ...
    using view_host_type = Kokkos::
            View<typename traits::data_type, typename traits::array_layout, typename traits::host_mirror_space, typename traits::memory_traits>;
    ...
    using view_type = Kokkos::View<
            ...
            typename traits::array_layout,
            std::conditional_t<AT == access_target::host, typename traits::host_mirror_space, typename traits::memory_space>,
            typename traits::memory_traits>;
...
private:
    ...
    [[nodiscard]] static auto get_access_target() noexcept -> access_target
    {
        if constexpr (std::is_same_v<typename Space::memory_space, typename view_device_type::memory_space>) {
            return access_target::device;
        } else if constexpr (std::is_same_v<typename Space::memory_space, typename view_host_type::memory_space>) {
            return access_target::host;
        }

        return access_target::invalid;
    }

    template <access_target AT>
    [[nodiscard]] auto get_view(AccessTargetTag<AT> const target) const
    {
        if constexpr (target.value == access_target::device) {
            return m_view_device;
        } else if constexpr (target.value == access_target::host) {
            return m_view_host;
        }
    }

    template <access_target AT>
    void mark_modified(AccessTargetTag<AT> const target) const noexcept
    {
        assert(is_allocated());
    ...
    auto copy_to(access_target const at) const -> void
    {
        if (at == access_target::device) {
    ...
    [[nodiscard]] auto access(AccessTag<AT, AM> const target_and_mode) const -> view_type<AM, AT>
    {
        ...
        if constexpr (need_read_access(get_mode(target_and_mode))) {
            sync(get_target(target_and_mode));
        }

        if constexpr (need_write_access(get_mode(target_and_mode))) {
...
public:
    ...
    template <access_target AT>
    [[nodiscard]] auto need_sync(AccessTargetTag<AT> const target) const noexcept -> bool
    {
        assert(is_allocated());
    ...
    template <access_target AT>
    void sync(AccessTargetTag<AT> const target) const
    {
        assert(is_allocated());

        if (need_sync(target)) {
            copy_to(target.value);
            clear_sync_state();
        }
    }
    ...
    auto clear_sync_state() const noexcept -> void
    {
        assert(is_allocated());
    ...
    [[nodiscard]] auto const_view() const noexcept -> DualView<typename traits::const_data_type, DataProperties...>
    {
        return DualView<typename traits::const_data_type, DataProperties...>(*this);
    }
};
