...
Range::Range(std::array<int, 3> const& Cmin, std::array<int, 3> const& Cmax, int Ng) : Corner_min(Cmin), Corner_max(Cmax), Ng(Ng), NgEff(0), Nghost({0, 0, 0})
{
...
Range::Range(std::array<int, 2> const& rng_x, std::array<int, 2> const& rng_y, std::array<int, 2> const& rng_z, int const Nghost) : Range({rng_x[0], rng_y[0], rng_z[0]}, {rng_x[1], rng_y[1], rng_z[1]}, Nghost) {}
...
Kokkos::MDRangePolicy<Kokkos::Rank<3, Kokkos::Iterate::Left, Kokkos::Iterate::Left>> cell_mdrange(Range const& range)
{
    ...
    return Kokkos::MDRangePolicy<int, Kokkos::Rank<3, Kokkos::Iterate::Left, Kokkos::Iterate::Left>>(begin, end);
}
