...
double integrate(Range const& range, Grid const& grid, KV_cdouble_3d const& var)
{
    ...
    Kokkos::parallel_reduce("integration", cell_mdrange(range), KOKKOS_LAMBDA(int i, int j, int k, double& local_sum) { local_sum += var(i, j, k) * dv(i, j, k); }, Kokkos::Sum<double>(sum));
