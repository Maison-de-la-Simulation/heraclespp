...
public:
    explicit UniformGravity(KV_cdouble_1d g) : m_g(std::move(g)) {}

    KOKKOS_FORCEINLINE_FUNCTION
    double operator()([[maybe_unused]] int i, [[maybe_unused]] int j, [[maybe_unused]] int k, int dir) const
    {
        return m_g(dir);
    }
};

inline UniformGravity make_uniform_gravity(Param const& param)
{
...
public:
    explicit PointMassGravity(KV_cdouble_1d g) : m_g(std::move(g)) {}

    KOKKOS_FORCEINLINE_FUNCTION
    double operator()(int i, [[maybe_unused]] int j, [[maybe_unused]] int k, int dir) const
    {
        if (dir == 0) {
            return m_g(i);
        }
...
inline PointMassGravity make_point_mass_gravity(Param const& param, Grid const& grid)
{
    ...
    Kokkos::parallel_for("point_mass_gravity", Kokkos::RangePolicy<int>(0, grid.Nx_local_wg[0]), KOKKOS_LAMBDA(int i) { g_array(i) = -units::G * M / (xc(i) * xc(i)); });
...
public:
    explicit InternalMassGravity(KV_cdouble_1d g) : m_g(std::move(g)) {}
...
inline InternalMassGravity make_internal_mass_gravity(Param const& param, Grid const& grid, KV_cdouble_3d const& rho)
{
    if (grid.mpi_dims_cart[0] != 1) {
        throw std::runtime_error(
                "The function make_internal_mass_gravity does not handle more than 1 MPI process "
    ...
    KDV_double_1d rho_mean_dv("rho_mean_array",
                              grid.Nx_local_ng[0] + grid.Nghost[0]); // sum (rho * dv) / (sum dv)
    ...
    for (int i = 0; i < grid.Nx_local_ng[0] + grid.Nghost[0]; ++i) {
        Kokkos::parallel_reduce(
                "integration_shell",
                Kokkos::MDRangePolicy<int, Kokkos::Rank<2>>({0, 0}, {grid.Nx_local_ng[1], grid.Nx_local_ng[2]}),
    ...
    Kokkos::parallel_for("rho_mean_o_dv_tot", Kokkos::RangePolicy<int>(0, grid.Nx_local_ng[0] + grid.Nghost[0]), KOKKOS_LAMBDA(int i) { rho_mean(i) /= dv_total(i); });

    Kokkos::parallel_for("ghost_cells_Mr", Kokkos::RangePolicy<int>(0, grid.Nghost[0]), KOKKOS_LAMBDA(int i) { M_r(i) = M_star; });
    ...
    Kokkos::parallel_scan(
            ...
            KOKKOS_LAMBDA(int i, double& partial_sum, bool is_final) {
                ...
                if (i == nghost[0]) {
                    ...
                    partial_sum += 4. / 3 * units::pi * (x3 - xc(i - 1) * xc(i - 1) * xc(i - 1)) * rho_mean(offset - 1) + 4. / 3 * units::pi * (xc(i) * xc(i) * xc(i) - x3) * rho_mean(offset);
                }
                if (is_final) {
                    M_r(i) = partial_sum;
                }
    ...
    Kokkos::parallel_for("internal_mass_gravity", Kokkos::RangePolicy<int>(0, grid.Nx_local_wg[0]), KOKKOS_LAMBDA(int i) { g_array_dv(i) = -units::G * M_r(i) / (xc(i) * xc(i)); });
