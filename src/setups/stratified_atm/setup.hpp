...
public:
    ...
    explicit ParamSetup(INIReader const& reader) : rho0(reader.GetReal("Initialisation", "rho0", 1.0)), u0(reader.GetReal("Initialisation", "u0", 1.0)), T(reader.GetReal("Perfect Gas", "temperature", 1.0)), gx(reader.GetReal("Gravity", "gx", 1.0)) {}
};
...
public:
    InitializationSetup(EOS const& eos, ParamSetup const& param_setup, Gravity gravity) : m_eos(eos), m_param_setup(param_setup), m_gravity(std::move(gravity)) {}
    ...
    void execute(Range const& range, Grid const& grid, KV_double_3d const& rho, KV_double_4d const& u, KV_double_3d const& P, [[maybe_unused]] KV_double_4d const& fx) const final
    {
        ...
        std::cout << "Scale = " << units::kb * m_param_setup.T / (mu * units::mp * m_param_setup.gx) << std::endl;

        Kokkos::parallel_for(
                "stratified_atm_init",
                cell_mdrange(range),
                KOKKOS_LAMBDA(int i, int j, int k) {
                    double const x0 = units::kb * param_setup.T / (mu * units::mp * Kokkos::fabs(gravity(i, j, k, 0)));
...
public:
    BoundarySetup(int idim, int iface, [[maybe_unused]] EOS const& eos, [[maybe_unused]] ParamSetup const& param_setup) : IBoundaryCondition<Gravity>(idim, iface), m_label(std::string("UserDefined").append(bc_dir(idim)).append(bc_face(iface))), m_eos(eos), m_param_setup(param_setup) {}

    void execute(Grid const& grid, [[maybe_unused]] Gravity const& gravity, KV_double_3d const& rho, KV_double_4d const& rhou, KV_double_3d const& E, [[maybe_unused]] KV_double_4d const& fx) const final
    {
        ...
        Kokkos::parallel_for(
                ...
                KOKKOS_LAMBDA(int i, int j, int k) {
                    ...
                    double const x0 = units::kb * param_setup.T * units::Kelvin / (mu * units::mp * Kokkos::fabs(gravity_x));
