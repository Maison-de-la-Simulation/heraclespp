...
public:
    InitializationSetup(thermodynamics::PerfectGas const& /*eos*/, ParamSetup const& param_set_up, Gravity gravity)
            ...
            const final
    {
        ...
        Kokkos::parallel_for(
                ...
                KOKKOS_LAMBDA(int i, int j, int k) {
                    double const P0
                            = (2 * units::pi * (param_setup.rho0 + param_setup.rho1) * Kokkos::fabs(gravity(i, j, k, 2)) * L) * units::pressure;
                    ...
                    for (int const kx : kx_array) {
                        for (int const ky : ky_array) {
                            ...
                            if (K >= Kmin && K <= Kmax) {
                                h += ((ak * Kokkos::cos(kx * X) * Kokkos::cos(ky * Y)) + (bk * Kokkos::cos(kx * X) * Kokkos::sin(ky * Y))
                                      + (ck * Kokkos::sin(kx * X) * Kokkos::cos(ky * Y)) + (dk * Kokkos::sin(kx * X) * Kokkos::sin(ky * Y)));
                            }
                        }
                    }
                    ...
                    if (z >= h) {
                        rho(i, j, k) = param_setup.rho0
                                       * Kokkos::
                                               pow(1 - ((gamma - 1) / gamma * (param_setup.rho0 * Kokkos::fabs(gravity(i, j, k, 2)) * z) / P0),
                                                   1. / (gamma - 1))
                                       * units::density;
                        ...
                        rho(i, j, k) = param_setup.rho1
                                       * Kokkos::
                                               pow(1 - ((gamma - 1) / gamma * (param_setup.rho1 * Kokkos::fabs(gravity(i, j, k, 2)) * z) / P0),
                                                   1. / (gamma - 1))
                                       * units::density;
