...
public:
    ...
    explicit ParamSetup(INIReader const& reader) : rho0(reader.GetReal("Initialisation", "rho0", 1.0)), rho1(reader.GetReal("Initialisation", "rho1", 1.0)), u0(reader.GetReal("Initialisation", "u0", 1.0)), P0(reader.GetReal("Initialisation", "P0", 1.0)) {}
};
...
public:
    InitializationSetup(thermodynamics::PerfectGas const& eos, ParamSetup const& param_set_up, Gravity gravity) : m_eos(eos), m_param_setup(param_set_up), m_gravity(std::move(gravity)) {}
    ...
    void execute(Range const& range, Grid const& grid, KV_double_3d const& rho, KV_double_4d const& u, KV_double_3d const& P, KV_double_4d const& fx) const final
    {
        ...
        Kokkos::parallel_for(
                ...
                KOKKOS_LAMBDA(int i, int j, int k) {
                    double const P0 = (2 * units::pi * (param_setup.rho0 + param_setup.rho1) * Kokkos::fabs(gravity(i, j, k, 2)) * L) * units::pressure;
                    ...
                    for (std::size_t ik = 0; ik < Kokkos::Array<int, 5>::size(); ++ik) {
                        for (std::size_t jk = 0; jk < Kokkos::Array<int, 5>::size(); ++jk) {
                            ...
                            if (K >= 8 && K <= 16) {
                                h += (ak * Kokkos::cos(kx[ik] * X) * Kokkos::cos(ky[jk] * Y) + bk * Kokkos::cos(kx[ik] * X) * Kokkos::sin(ky[jk] * Y) + ck * Kokkos::sin(kx[ik] * X) * Kokkos::cos(ky[jk] * Y) + dk * Kokkos::sin(kx[ik] * X) * Kokkos::sin(ky[jk] * Y));
                            }
                        }
                    }

                    if (z >= h) {
                        rho(i, j, k) = param_setup.rho0 * Kokkos::pow(1 - (gamma - 1) / gamma * (param_setup.rho0 * Kokkos::fabs(gravity(i, j, k, 2)) * z) / P0, 1. / (gamma - 1)) * units::density;
                    ...
                    if (z < h) {
                        rho(i, j, k) = param_setup.rho1 * Kokkos::pow(1 - (gamma - 1) / gamma * (param_setup.rho1 * Kokkos::fabs(gravity(i, j, k, 2)) * z) / P0, 1. / (gamma - 1)) * units::density;
