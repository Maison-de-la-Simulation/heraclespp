...
public:
    ...
    explicit ParamSetup(INIReader const& reader) : rho0(reader.GetReal("Initialisation", "rho0", 1.0)), u0(reader.GetReal("Initialisation", "u0", 1.0)), T(reader.GetReal("Initialisation", "T", 100.)), M(reader.GetReal("Gravity", "M", 1.0)) {}
};
...
public:
    InitializationSetup(EOS const& eos, ParamSetup const& param_setup, [[maybe_unused]] Gravity const& gravity) : m_eos(eos), m_param_setup(param_setup) {}
    ...
    void execute(Range const& range, Grid const& grid, KV_double_3d const& rho, KV_double_4d const& u, KV_double_3d const& P, [[maybe_unused]] KV_double_4d const& fx) const final
    {
        ...
        std::cout << "Scale = " << units::kb * m_param_setup.T / (mu * units::mp * units::G * m_param_setup.M) << "\n";
...
public:
    BoundarySetup(int idim, int iface, EOS const& eos, ParamSetup const& param_setup) : IBoundaryCondition<Gravity>(idim, iface), m_label(std::string("UserDefined").append(bc_dir(idim)).append(bc_face(iface))), m_eos(eos), m_param_setup(param_setup) {}
