...
void main(int argc, char** argv)
{
    ...
    for (int idim = 0; idim < ndim; ++idim) {
        ...
        for (int iface = 0; iface < 2; ++iface) {
            bc_choice_faces[idim * 2 + iface] = reader.Get("Boundary Condition", std::string("BC").append(bc_dir(idim)).append(bc_face(iface)), bc_choice_dir);
    ...
    KDV_double_4d rhou("rhou", grid.Nx_local_wg[0], grid.Nx_local_wg[1], grid.Nx_local_wg[2],
                       ndim); // Momentum
    ...
    {
        read_pdi(param.restart_file, output_id, iter_output_id, time_output_id, iter, t, rho, u, P, fx, x_glob, y_glob,
                 z_glob); // read data into host view
    ...
    for (int idim = 0; idim < ndim; ++idim) {
        for (int iface = 0; iface < 2; ++iface) {
            if (bc_choice_faces[idim * 2 + iface] == "UserDefined") {
                ...
                bcs_array[idim * 2 + iface] = factory_boundary_construction<Gravity>(bc_choice_faces[idim * 2 + iface], idim, iface);
            }
        }
    }
    ...
    std::unique_ptr<IFaceReconstruction> face_reconstruction = factory_face_reconstruction(param.reconstruction_type);

    std::unique_ptr<IExtrapolationReconstruction<Gravity>> time_reconstruction = std::make_unique<ExtrapolationTimeReconstruction<EOS, Gravity>>(eos);

    std::unique_ptr<IHydroReconstruction<Gravity>> reconstruction = std::make_unique<MUSCLHancockHydroReconstruction<EOS, Gravity>>(std::move(face_reconstruction), std::move(time_reconstruction), eos, P_rec, u_rec);

    std::unique_ptr<IGodunovScheme<Gravity>> godunov_scheme = factory_godunov_scheme<EOS, Gravity>(param.riemann_solver, eos);
    ...
    while (!should_exit) {
        ...
        reconstruction->execute(grid.range.with_ghosts(1), grid, *g, dt / 2, rho.view_device(), u.view_device(), P.view_device(), fx.view_device(), rho_rec, rhou_rec, E_rec, fx_rec);

        godunov_scheme->execute(grid.range.no_ghosts(), grid, *g, dt, rho.view_device(), rhou.view_device(), E.view_device(), fx.view_device(), rho_rec, rhou_rec, E_rec, fx_rec, rho_new, rhou_new, E_new, fx_new);

        if (param.pressure_fix == "On") {
            pressure_fix(grid.range.no_ghosts(), eos, grid, dt, param.eps_pf, rho.view_device(), rhou.view_device(), E.view_device(), rho_rec, rhou_rec, E_rec, rho_new, rhou_new, E_new);
        }
        ...
        if (shift_criterion->execute(grid.range.no_ghosts(), grid, rho_new, rhou_new, E_new, fx_new)) {
            ...
            if (x_h(grid.Nx_glob_ng[0] + grid.Nghost[0] + 1) >= param.rmax_shift) {
                ...
                bcs_array[0 * 2 + 1] = factory_boundary_construction<Gravity>("NullGradient", 0, 1);
                DistributedBoundaryCondition const bcs(grid, param);
                ...
                shift_grid(rho.view_device(), rhou.view_device(), E.view_device(), fx.view_device(), rho_new, rhou_new, E_new, fx_new, x_glob, y_glob, z_glob, grid);
                bcs(bcs_array, grid, *g, rho_new, rhou_new, E_new, fx_new);
            }
