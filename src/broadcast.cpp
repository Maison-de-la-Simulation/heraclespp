...
void broadcast(Range const& range, double const in, KV_double_3d const& out)
{
    ...
    Kokkos::parallel_for("broadcast 0d->3d", cell_mdrange(range), KOKKOS_LAMBDA(const int i, const int j, const int k) { out(i, j, k) = in; });
}
...
void broadcast(Range const& range, KV_cdouble_1d const& in, KV_double_3d const& out)
{
    ...
    Kokkos::parallel_for("broadcast 1d->3d", cell_mdrange(range), KOKKOS_LAMBDA(const int i, const int j, const int k) { out(i, j, k) = in(i - ghost_x); });
}
