...
public:
    ...
    virtual void execute(Range const& range, Grid const& grid, Gravity const& gravity, double dt_reconstruction, KV_cdouble_6d const& u_rec, KV_cdouble_5d const& P_rec, KV_double_5d const& rho_rec, KV_double_6d const& rhou_rec, KV_double_5d const& E_rec, KV_double_6d const& fx_rec) const = 0;
};

template <class EoS, class Gravity>
class ExtrapolationTimeReconstruction : public IExtrapolationReconstruction<Gravity>
{
    static_assert(std::is_invocable_r_v<void, Gravity, int, int, int, int>, "Incompatible gravity.");

private:
    EoS m_eos;

public:
    explicit ExtrapolationTimeReconstruction(EoS const& eos) : m_eos(eos) {}

    void execute(Range const& range, Grid const& grid, Gravity const& gravity, double const dt_reconstruction, KV_cdouble_6d const& u_rec, KV_cdouble_5d const& P_rec, KV_double_5d const& rho_rec, KV_double_6d const& rhou_rec, KV_double_5d const& E_rec, KV_double_6d const& fx_rec) const final
    {
                ...
                KOKKOS_LAMBDA(int i, int j, int k) {
                    ...
                    for (int idim = 0; idim < ndim; ++idim) {
                        ...
                        for (int ipos = 0; ipos < ndim; ++ipos) {
                            ...
                            for (int idr = 0; idr < ndim; ++idr) {
                                rhou_rec(i, j, k, 0, ipos, idr) += dtodv * (fluxL.rhou[idr] * ds(i, j, k, idim) - fluxR.rhou[idr] * ds(i_p, j_p, k_p, idim));
                                rhou_rec(i, j, k, 1, ipos, idr) += dtodv * (fluxL.rhou[idr] * ds(i, j, k, idim) - fluxR.rhou[idr] * ds(i_p, j_p, k_p, idim));
                            }
                        ...
                        if (geom == Geometry::Geom_spherical) {
                            for (int ipos = 0; ipos < ndim; ++ipos) {
                                if (idim == 0) {
                                    ...
                                    rhou_rec(i, j, k, 0, ipos, idim) += source_grad_P(dtodv, primL.P, primR.P, ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));
                                    rhou_rec(i, j, k, 1, ipos, idim) += source_grad_P(dtodv, primL.P, primR.P, ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));

                                    if (ndim == 2 || ndim == 3) {
                                        // Velocity term (e_{r}): rho * u_{th} * u_{th} / r
                                        rhou_rec(i, j, k, 0, ipos, idim) += source_grad_u_r(dtodv, primL.rho, primR.rho, primL.u[1], primR.u[1], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));
                                        rhou_rec(i, j, k, 1, ipos, idim) += source_grad_u_r(dtodv, primL.rho, primR.rho, primL.u[1], primR.u[1], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));

                                        // Velocity term (e_{th}): rho * u_{th} * u_{r} / r
                                        rhou_rec(i, j, k, 0, ipos, 1) -= source_grad_u_idir_r(dtodv, x(i), x(i + 1), primL.rho, primR.rho, primL.u[0], primR.u[0], primL.u[1], primR.u[1], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));
                                        rhou_rec(i, j, k, 1, ipos, 1) -= source_grad_u_idir_r(dtodv, x(i), x(i + 1), primL.rho, primR.rho, primL.u[0], primR.u[0], primL.u[1], primR.u[1], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));
                                    }
                                    if (ndim == 3) {
                                        // Velocity term (e_{r}): rho * u_{phi} * u_{phi} / r
                                        rhou_rec(i, j, k, 0, ipos, idim) += source_grad_u_r(dtodv, primL.rho, primR.rho, primL.u[2], primR.u[2], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));
                                        rhou_rec(i, j, k, 1, ipos, idim) += source_grad_u_r(dtodv, primL.rho, primR.rho, primL.u[2], primR.u[2], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));

                                        // Velocity term (e_{phi}): rho * u_{phi} * u_{r} / r
                                        rhou_rec(i, j, k, 0, ipos, 2) -= source_grad_u_idir_r(dtodv, x(i), x(i + 1), primL.rho, primR.rho, primL.u[0], primR.u[0], primL.u[2], primR.u[2], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));
                                        rhou_rec(i, j, k, 1, ipos, 2) -= source_grad_u_idir_r(dtodv, x(i), x(i + 1), primL.rho, primR.rho, primL.u[0], primR.u[0], primL.u[2], primR.u[2], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));
                                    }
                                }
                                if (idim == 1) {
                                    ...
                                    if (ndim == 3) {
                                        ...
                                        rhou_rec(i, j, k, 0, ipos, idim) += source_grad_u_th(dtodv, y(j), y(j + 1), primL.rho, primR.rho, primL.u[2], primR.u[2], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));
                                        rhou_rec(i, j, k, 1, ipos, idim) += source_grad_u_th(dtodv, y(j), y(j + 1), primL.rho, primR.rho, primL.u[2], primR.u[2], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));

                                        // Velocity term (e_{phi}): cot(th) * rho * u_{phi} * u_{th} / r
                                        rhou_rec(i, j, k, 0, ipos, 2) -= source_grad_u_phi(dtodv, y(j), y(j + 1), primL.rho, primR.rho, primL.u[1], primR.u[1], primL.u[2], primR.u[2], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));
                                        rhou_rec(i, j, k, 1, ipos, 2) -= source_grad_u_phi(dtodv, y(j), y(j + 1), primL.rho, primR.rho, primL.u[1], primR.u[1], primL.u[2], primR.u[2], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));
                                    }
                                }
                            }
                        }
