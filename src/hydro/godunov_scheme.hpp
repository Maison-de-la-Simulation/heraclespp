                ...
                for (int idim = 0; idim < ndim; ++idim)
                {
                    ...
                    if (geom == Geometry::Geom_spherical)
                    {
                        ...
                        if (idim == 0)
                        {
                            ...
                            rhou_new(i, j, k, idim) += source_grad_P(dtodv, primL, primR, dsL, dsR);
                            if (ndim == 2 || ndim == 3)
                            {
                                // Velocity term (e_{r}): rho * u_{th} * u_{th} / r
                                rhou_new(i, j, k, idim) += source_grad_u_r(1, dtodv, primL, primR, dsL, dsR);

                                // Velocity term (e_{th}): rho * u_{th} * u_{r} / r
                                rhou_new(i, j, k, 1) -= source_grad_u_idir_r(1, dtodv, x(i), x(i+1), primL, primR, dsL, dsR);
                            }
                            if (ndim == 3)
                            {
                                // Velocity term (e_{r}): rho * u_{phi} * u_{phi} / r
                                rhou_new(i, j, k, idim) += source_grad_u_r(2, dtodv, primL, primR, dsL, dsR);

                                // Velocity term (e_{phi}): rho * u_{phi} * u_{r} / r
                                rhou_new(i, j, k, 2) -= source_grad_u_idir_r(2, dtodv, x(i), x(i+1), primL, primR, dsL, dsR);
                            }
                        }
                        if (idim == 1)
                        {
                            ...
                            if (ndim == 3)
                            {
                                ...
                                rhou_new(i, j, k, idim) += source_grad_u_th(dtodv, y(j), y(j+1), primL, primR, dsL, dsR);

                                // Velocity term (e_{phi}): cot(th) * rho * u_{phi} * u_{th} / r
                                rhou_new(i, j, k, 2) -= source_grad_u_phi(dtodv, y(j), y(j+1), primL, primR, dsL, dsR);
                            }
                        }
                    }
