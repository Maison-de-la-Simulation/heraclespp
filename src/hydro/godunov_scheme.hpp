...
{
    static_assert(std::is_invocable_r_v<EulerFlux, RiemannSolver, EulerCons, EulerCons, int, EoS>, "Incompatible Riemann solver.");
...
public:
    RiemannBasedGodunovScheme(RiemannSolver const& riemann_solver, EoS const& eos) : m_riemann_solver(riemann_solver), m_eos(eos) {}
                    ...
                    for (int idim = 0; idim < ndim; ++idim) {
                        ...
                        if (geom == Geometry::Geom_spherical) {
                            ...
                            if (idim == 0) {
                                ...
                                rhou_new(i, j, k, idim) += source_grad_P(dtodv, primL.P, primR.P, ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));

                                if (ndim == 2 || ndim == 3) {
                                    // Velocity term (e_{r}): rho * u_{th} * u_{th} / r
                                    rhou_new(i, j, k, idim) += source_grad_u_r(dtodv, primL.rho, primR.rho, primL.u[1], primR.u[1], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));

                                    // Velocity term (e_{th}): rho * u_{th} * u_{r} / r
                                    rhou_new(i, j, k, 1) -= source_grad_u_idir_r(dtodv, x(i), x(i + 1), primL.rho, primR.rho, primL.u[0], primR.u[0], primL.u[1], primR.u[1], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));
                                }
                                if (ndim == 3) {
                                    // Velocity term (e_{r}): rho * u_{phi} * u_{phi} / r
                                    rhou_new(i, j, k, idim) += source_grad_u_r(dtodv, primL.rho, primR.rho, primL.u[2], primR.u[2], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));

                                    // Velocity term (e_{phi}): rho * u_{phi} * u_{r} / r
                                    rhou_new(i, j, k, 2) -= source_grad_u_idir_r(dtodv, x(i), x(i + 1), primL.rho, primR.rho, primL.u[0], primR.u[0], primL.u[2], primR.u[2], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));
                                }
                            }
                            if (idim == 1) {
                                ...
                                if (ndim == 3) {
                                    ...
                                    rhou_new(i, j, k, idim) += source_grad_u_th(dtodv, y(j), y(j + 1), primL.rho, primR.rho, primL.u[2], primR.u[2], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));

                                    // Velocity term (e_{phi}): cot(th) * rho * u_{phi} * u_{th} / r
                                    rhou_new(i, j, k, 2) -= source_grad_u_phi(dtodv, y(j), y(j + 1), primL.rho, primR.rho, primL.u[1], primR.u[1], primL.u[2], primR.u[2], ds(i, j, k, idim), ds(i_p, j_p, k_p, idim));
                                }
                            }
                        }
...
inline std::unique_ptr<IGodunovScheme<Gravity>> factory_godunov_scheme(std::string const& riemann_solver, EoS const& eos)
{
    ...
    if (riemann_solver == "HLLC") {
        return std::make_unique<RiemannBasedGodunovScheme<HLLC, Gravity, EoS>>(HLLC(), eos);
    }

    if (riemann_solver == "Low Mach") {
        return std::make_unique<RiemannBasedGodunovScheme<Splitting, Gravity, EoS>>(Splitting(), eos);
    }
