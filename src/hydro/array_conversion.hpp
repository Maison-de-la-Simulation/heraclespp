...
template <class EoS>
void conv_cons_to_prim(Range const& range, EoS const& eos, KV_cdouble_3d const& rho, KV_cdouble_4d const& rhou, KV_cdouble_3d const& E, KV_double_4d const& u, KV_double_3d const& P)
{
    assert(equal_extents({0, 1, 2}, rho, rhou, E, u, P));
    assert(equal_extents(3, rhou, u));
    assert(u.extent_int(3) == ndim);
