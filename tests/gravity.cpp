...
void test_gravity_internal_gravity()
{
    ...
    regular_grid.execute(
            grid.Nghost,
            grid.Nx_glob_ng,
            x0_glob(hclpp::host, hclpp::read_write),
            x1_glob(hclpp::host, hclpp::read_write),
            x2_glob(hclpp::host, hclpp::read_write));
    ...
    hclpp::InternalMassGravity const g = hclpp::make_internal_mass_gravity(M_star, grid, rho);

    // Theoretical gravitational field
    auto x0c = grid.x0_center;
    hclpp::KV_double_1d const g_th("g_th", grid.Nx_local_wg[0]);
...
void test_gravity_uniform_gravity()
{
    ...
    {
        auto const g_h = g(hclpp::host, hclpp::read_write);
        g_h(0) = gx;
        g_h(1) = gy;
        g_h(2) = gz;
    }
    ...
    {
        auto const result_d = result(hclpp::device, hclpp::read_write);
        Kokkos::parallel_for(
