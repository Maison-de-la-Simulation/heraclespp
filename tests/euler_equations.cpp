...
TEST(EulerEquations, KineticEnergyFromCons)
{
    ...
    hclpp::EulerCons const cons {.rho = rho0, .rhou = {rhou0}, .E = E0};
...
TEST(EulerEquations, FluxFromPrim)
{
    ...
    hclpp::EulerPrim const prim {.rho = rho0, .u = {u0}, .P = P0};
    hclpp::EulerFlux const flux = hclpp::compute_flux(prim, locdim, eos);
...
TEST(EulerEquations, FluxFromCons)
{
    ...
    hclpp::thermodynamics::PerfectGas const eos(gamma, mmw);
    hclpp::EulerCons const cons {.rho = rho0, .rhou = {rhou0}, .E = E0};
    hclpp::EulerFlux const flux = hclpp::compute_flux(cons, locdim, eos);
    EXPECT_DOUBLE_EQ(flux.rho, 20.);
    EXPECT_DOUBLE_EQ(flux.E, 1000.);
    EXPECT_DOUBLE_EQ(flux.rhou[0], 200.);
}
...
TEST(EulerEquations, PrimToConsToPrim)
{
    ...
    hclpp::thermodynamics::PerfectGas const eos(gamma, mmw);
    hclpp::EulerPrim const prim {.rho = rho0, .u = {u0}, .P = P0};
    hclpp::EulerPrim const prim_to_cons_to_prim = to_prim(to_cons(prim, eos), eos);
